...
inline void
moveTo(Transform& transform, const std::pair<int, int> nextPos, const int tMs) {
...
inline void setAt(Transform& transform, const std::pair<int, int> pos) {
  const auto [x, y] = pos;
  transform.x = x;
  transform.y = y;
  transform.prevX = x;
...
inline bool isMoving(const Transform& transform) { return transform.isMoving; }

inline void update(Transform& transform, const int dt) {
